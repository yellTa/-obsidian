...
const pages = dv.pages('[[1_Projects/제로베이스 대기업 취업 특별반/실전프로젝트/문서]]') .where(p => p.file.name !== "문서") .sort(p => p.created, 'asc'); // created 태그 기준으로 오름차순 정렬
