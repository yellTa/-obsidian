...
dv.table(
  ...
  pages.map(p => {
    ...
    if (progress.toLowerCase() === "unsolved") {
      fileName = dv.fileLink(p.file.path, `<span style="color: red;">${p.file.name}</span>`);
    } else if (progress.toLowerCase() === "ongoing") {
      fileName = dv.fileLink(p.file.path, `<span style="color: orange;">${p.file.name}</span>`);
    } else {
